...
impl Evaluation for DeveloperSearchReplace {
    async fn run(
        ...
        let assets_exists = assets_dir_path.exists();
        ...
        let target_file = std::env::current_dir().unwrap_or_default().join("kubernetes_swagger.json");
        ...
        let modified_file_path = std::env::current_dir().unwrap_or_default().join("kubernetes_swagger.json");
        ...
        let actual_diff = String::from_utf8_lossy(&diff_output.stdout).to_string().lines().skip(4).collect::<Vec<&str>>().join("\n");
        ...
        metrics.push(("Changes match expected patch".to_string(), EvaluationMetric::Boolean(changes_match)));
