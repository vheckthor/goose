...
pub fn render_left_list(f: &mut Frame, header: String, items: Vec<String>, list_state: &mut ListState, is_focussed_view: bool, area: Rect) {
        // Main - left list area
        ...
        let list_header = Paragraph::new(vec![
            Line::from(vec![Span::raw("   "), Span::styled(header, Style::default().add_modifier(Modifier::ITALIC))]),
            Line::from(vec!["".into()])
