...
use ratatui::{crossterm::event::{Event, KeyCode, KeyEvent}, layout::{self, Layout, Rect}, style::{Color, Modifier, Style}, text::{Line, Span}, widgets::{Block, Borders, Clear, HighlightSpacing, List, ListState, Paragraph}, Frame};
use tui_input::{backend::crossterm::EventHandler, Input};

use crate::profile::{load_profiles, remove_profile, save_profile, Profile};

use super::{main_area::{chunks_for_list_and_view_split, render_left_list}, provider_list, AppOutcome};
...
impl ProfileUI {
                        ...
                        match key.code {
                            ...
                            KeyCode::Enter => { // Change to save key
                                if let Some(edit_profile) = &self.edit_profile {
                                    ...
                                    self.profiles.insert(edit_profile.name.value().to_string(), new_profile.clone());
    ...
    pub fn render_main_area(&mut self, f: &mut Frame, main_area: Rect, view_focussed: bool) {
        ...
        render_left_list(f, "Profiles".to_string(), profile_list_names.clone(), &mut self.profile_list_state, view_focussed, main_area_horizontal_chunks[0]);

        // Main - Profile details area
        match self.profile_ui_mode {
            ProfileUIMode::ProfileView => {
                if has_profiles {
                    let (selected_profile_name, selected_profile) = self.selected_profile().unwrap();
    ...
    fn selected_profile(&self) -> Option<(&String, &Profile)> {
        ...
        let target_profile_name = profile_names.get(self.profile_list_state.selected().unwrap_or(0)).unwrap();
        Some(self.profiles.iter().find(|(name, _)| target_profile_name == *name).map(|(name, profile)| (name, profile)).unwrap())
    }
