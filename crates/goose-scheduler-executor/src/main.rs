...
async fn execute_recipe(job_id: &str, recipe_path: &str) -> Result<String> {
    ...
    let provider_name: String = global_config
        ...
        .map_err(|_| anyhow!("GOOSE_PROVIDER not configured. Run 'goose configure' or set env var."))?;
    let model_name: String = global_config
        .get_param("GOOSE_MODEL")
        .map_err(|_| anyhow!("GOOSE_MODEL not configured. Run 'goose configure' or set env var."))?;
    ...
    info!("Agent configured with provider '{}' for job '{}'", provider_name, job_id);
    ...
    let Some(prompt_text) = recipe.prompt else {
        info!("Recipe '{}' has no prompt to execute for job '{}'", recipe_path, job_id);
    ...
    let mut stream = agent.reply(&messages, Some(session_config)).await
    ...
    info!("Finished executing job '{}', session: {}", job_id, session_id);
    Ok(session_id)
}