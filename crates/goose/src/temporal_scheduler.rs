...
impl TemporalScheduler {
    ...
    async fn start_temporal_server(&self) -> Result<(), SchedulerError> {
        ...
        if !output.status.success() {
            return Err(SchedulerError::SchedulerInternalError(
                format!("Failed to start Temporal server: {}", String::from_utf8_lossy(&output.stderr))
            ));
        }
    ...
    async fn start_go_service(&self) -> Result<(), SchedulerError> {
        ...
        let output = Command::new("sh")
            .arg("-c")
            .arg(&format!("cd temporal-service && nohup ./temporal-service > temporal-service.log 2>&1 & echo $!"))
            .output()
            .map_err(|e| SchedulerError::SchedulerInternalError(
        ...
        if !output.status.success() {
            return Err(SchedulerError::SchedulerInternalError(
                format!("Failed to start Go service: {}", String::from_utf8_lossy(&output.stderr))
            ));
        }
...
impl TemporalScheduler {
    ...
    pub async fn get_service_info(&self) -> String {
        ...
        format!(
            ...
            if temporal_running { "localhost" } else { "not running" },
            if temporal_running { "✅ Running" } else { "❌ Not Running" },
            if go_running { "localhost" } else { "not running" },
            if go_running { "✅ Running" } else { "❌ Not Running" }
        )
    }
